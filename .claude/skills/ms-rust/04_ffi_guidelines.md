...
- âš ï¸ any allocated instance, e.g., `String`, `Vec<u8>`, `Box<Foo>`, ...
- âš ï¸ any library relying on other statics, e.g., `tokio`, `log`,
- âš ï¸ any struct not `#[repr(C)]`,
- âš ï¸ any data structure relying on consistent `TypeId`.
...
fn use_common_service(common: &CommonService) {
    ...
    //   DLL2, but will use statics from DLL1 âš ï¸
    common.do_work();
}
```
