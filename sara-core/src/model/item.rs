...
impl ItemType {
    ...
    /// Generates an ID for this item type with an optional sequence number.
    ///
    /// If no sequence is provided, defaults to 1.
    ///
    /// # Examples
    /// ```
    ...
    /// Generates a default name from an ID.
    ///
    /// Extracts the numeric suffix from the ID and combines it with the
    /// display name. For example, "SOL-001" becomes "Solution 001".
    #[must_use]
    pub fn default_name(&self, id: &str) -> String {
        let suffix = id
    ...
    pub const fn requires_satisfies(&self) -> bool {
        matches!(self, Self::SystemArchitecture) || self.is_detailed_design()
    }
    ...
    /// Returns true if this is a requirement type (System, Software, or Hardware).
    #[must_use]
    pub const fn is_requirement(&self) -> bool {
        matches!(
            self,
            Self::SystemRequirement | Self::SoftwareRequirement | Self::HardwareRequirement
        )
    }

    /// Returns true if this is a detailed design type (Software or Hardware).
    #[must_use]
    pub const fn is_detailed_design(&self) -> bool {
        matches!(
            self,
            Self::SoftwareDetailedDesign | Self::HardwareDetailedDesign
        )
    }

    /// Returns true if this is an Architecture Decision Record type.
    #[must_use]
    pub const fn is_adr(&self) -> bool {
...
impl ItemId {
    ...
    /// Converts a slice of ItemIds to a vector of string references.
    #[must_use]
    pub fn slice_to_strs(ids: &[Self]) -> Vec<&str> {
...
impl ItemAttributes {
    ...
    /// Returns the depends_on references as Option (None for non-requirement types).
    #[must_use]
...
impl Item {
    /// Returns an iterator over all referenced item IDs (relationships and peer attributes).
    pub fn all_references(&self) -> impl Iterator<Item = &ItemId> {
        // Relationship targets
        let rel_targets = self.relationships.iter().map(|r| &r.to);
        ...
        rel_targets.chain(peer_refs)
    }

    /// Returns an iterator over upstream relationships (Refines, DerivesFrom, Satisfies, Justifies).
    pub fn upstream_relationships(&self) -> impl Iterator<Item = &Relationship> {
        self.relationships
            .iter()
            .filter(|r| r.relationship_type.is_upstream())
    }

    /// Returns an iterator over downstream relationships (IsRefinedBy, Derives, IsSatisfiedBy, IsJustifiedBy).
    pub fn downstream_relationships(&self) -> impl Iterator<Item = &Relationship> {
        self.relationships
            .iter()
    ...
    pub fn all_upstream_ids(&self) -> impl Iterator<Item = &ItemId> {
        self.upstream_relationships().map(|r| &r.to)
    }

    /// Returns an iterator over all downstream item IDs.
    pub fn all_downstream_ids(&self) -> impl Iterator<Item = &ItemId> {
        self.downstream_relationships().map(|r| &r.to)
    }

    /// Returns true if this item has no upstream relationships.
    #[must_use]
    ...
    /// Returns an iterator over item IDs for a specific relationship type.
    ...
    /// Checks if a relationship of given type to given target exists.
    #[must_use]
    pub fn has_relationship(&self, rel_type: RelationshipType, target: &ItemId) -> bool {
        self.relationships
            .iter()
            .any(|r| r.relationship_type == rel_type && r.to == *target)
    }

    /// Returns downstream relationships with their types (for redundant rule compatibility).
    pub fn downstream_iter_with_types(&self) -> impl Iterator<Item = (RelationshipType, &ItemId)> {
        self.downstream_relationships()
            .map(|r| (r.relationship_type, &r.to))
    }

    /// Checks if upstream contains a specific type and ID (for redundant rule compatibility).
    #[must_use]
    pub fn upstream_contains_for_type(&self, rel_type: RelationshipType, id: &ItemId) -> bool {
        self.relationships
            .iter()
            .any(|r| r.relationship_type == rel_type && r.to == *id)
    }
}
...
mod tests {
    ...
    fn test_generate_id() {
        ...
        assert_eq!(ItemType::SystemRequirement.generate_id(None), "SYSREQ-001");
        assert_eq!(
            ItemType::ArchitectureDecisionRecord.generate_id(Some(1)),
            "ADR-001"
        );
    }
}
