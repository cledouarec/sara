...
use crate::model::{AdrStatus, ItemType, TypeFields};
...
/// Convert RawFrontmatter to TypeFields.
///
/// This extracts the type-specific fields from the raw frontmatter
/// into the flat TypeFields container.
pub fn to_fields(raw: &RawFrontmatter) -> TypeFields {
    TypeFields {
        // Requirement fields
        ...
        // ADR fields
        status: raw.status,
        deciders: raw.deciders.clone(),
        justifies: raw.justifies.clone(),
...
impl RawFrontmatter {
    /// Converts to TypeFields.
    pub fn to_fields(&self) -> TypeFields {
        to_fields(self)
    }
}
...
"#;
        ...
        assert_eq!(fields.derives_from, vec!["SCEN-001"]);
        assert_eq!(fields.depends_on, vec!["SYSREQ-002"]);
    }
...
"#;
        ...
        assert_eq!(fields.status, Some(AdrStatus::Proposed));
        assert_eq!(fields.deciders, vec!["Alice", "Bob"]);
        assert_eq!(fields.justifies, vec!["SYSARCH-001"]);
    }
...
"#;
        ...
        assert_eq!(fields.platform, Some("AWS Lambda".to_string()));
        assert_eq!(fields.satisfies, vec!["SYSREQ-001"]);
    }
